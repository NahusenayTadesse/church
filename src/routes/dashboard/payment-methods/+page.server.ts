...
export const actions: Actions = {
	...
	edit: async ({ request, locals }) => {
		...
		const { id, name, logo } = form.data;

		try {   


			    const [oldLogo] = await db.select({ logo: paymentMethods.logo}).from(paymentMethods).where(eq(paymentMethods.id, id)).limit(1);
    			 const logoUrl =  logo ? await saveUploadedFile(logo) : oldLogo.logo;


			await db
				.update(paymentMethods)
				.set({ name, updatedBy: locals?.user?.id, logo: logoUrl })
