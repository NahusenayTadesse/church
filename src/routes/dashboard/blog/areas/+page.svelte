...
{#key data.allData}
	<DialogComp title="Add New Minstry Areas" IconComp={Plus} variant="default">
		<form action="?/add" use:enhance id="main" class="flex flex-col gap-4" method="post">
			...
			<InputComp
				...
				type="textarea"
				name="description"
				placeholder="Enter Minstry Areas Description"
				required={true}
				rows={10}
			/>
			...
			<Button type="submit" form="main">
				{#if $delayed}
					<LoadingBtn name="Adding Minstry Areas" />
				{:else}
					<Plus /> Add Minstry Areas
				{/if}
			</Button>
		</form>
	</DialogComp>
